    ...
    <div className="pt-2 pb-8">
      <div className="space-y-0">
        {latestComments.map((comment, i) => {
          ...
          const isUpvoted =
            optimisticUpvoted !== null ? optimisticUpvoted : baseUpvoted;
          const scoreDelta = (isUpvoted ? 1 : 0) - (baseUpvoted ? 1 : 0);
          const displayScore = comment.score + scoreDelta;

          return (
            <div
              key={comment.id}
              className="flex gap-1 md:gap-2 px-2 md:px-3 py-2 md:py-1 text-[13px] leading-tight"
            >
