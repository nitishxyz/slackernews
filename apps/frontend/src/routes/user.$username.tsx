...
export const Route = createFileRoute('/user/$username')({
  loader: async ({ params }) => {
    const user = await getUserByUsername({ data: { username: params.username } })
    if (!user) {
      throw notFound()
    }
    return { user }
...
function UserProfile() {
  ...
  return (
    <div className="px-2 py-2 max-w-4xl">
      <table className="text-[10pt] text-black border-collapse" style={{ borderSpacing: 0 }}>
        <tbody>
          <tr className="h-6">
            <td className="align-top text-[#828282] w-24 pr-2">user:</td>
            <td className="align-top font-bold text-[#3c963c]">{profileUser.username}</td>
          </tr>
          <tr className="h-6">
            <td className="align-top text-[#828282] w-24 pr-2">created:</td>
            <td className="align-top">{new Date(profileUser.createdAt).toLocaleDateString()}</td>
          </tr>
          <tr className="h-6">
            ...
            <>
              <tr className="h-6">
                ...
                <td className="align-top">
                  {isLoadingBalance ? 'loading...' : `$${balanceData?.balance?.toFixed(2) ?? '...'} USDC`}
                </td>
              </tr>
              <tr className="h-6">
          ...
          <tr className="h-6">
            ...
            <td className="align-top pt-4">
              <a href="#" className="underline text-black mr-2">submissions</a>
              <a href="#" className="underline text-black mr-2">comments</a>
              <a href="#" className="underline text-black">favorites</a>
            </td>
          </tr>
        </tbody>
      </table>
