...
import { fetchUserPosts } from '../server/posts'
import { timeAgo } from '../lib/utils'
import { z } from 'zod'

const SubmittedSearchSchema = z.object({
  page: z.number().default(1).catch(1),
})

export const Route = createFileRoute('/s/$username')({
  validateSearch: (search) => SubmittedSearchSchema.parse(search),
  loaderDeps: ({ search: { page } }) => ({ page }),
  loader: async ({ deps, params }) => {
    const { page } = deps;
    const username = params.username;
    const { posts, hasMore } = await fetchUserPosts({ data: { username, page, limit: 30 } })
    return { 
...
function SubmittedPage() {
  const { userPosts, hasMore, page, username } = Route.useLoaderData()
